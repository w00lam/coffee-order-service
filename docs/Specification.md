...
- 데이터 수집 플랫폼 Consumer는 사용자 식별값, 복수 항목과 총 결제금액을 HTTP `POST /orders`로 전달합니다. `eventId`를 `Idempotency-Key` header와 Body에 함께 보내 외부 플랫폼이 재시도 중복을 제거할 수 있게 합니다.
...
- 데이터 수집 플랫폼의 운영 URL, 인증 방식, 가용성과 장애 처리 계약
- Consumer 재시도 횟수와 간격
