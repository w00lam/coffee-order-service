$LoadTestConfig = [ordered]@{
    ComposeProject = if ($env:COMPOSE_PROJECT_NAME) { $env:COMPOSE_PROJECT_NAME } else { 'coffee-order-load-test' }
    ...
    Topic = if ($env:ORDER_EVENTS_TOPIC) { $env:ORDER_EVENTS_TOPIC } else { 'coffee-order-completed-load-test' }
    ConsumerGroup = if ($env:POPULAR_MENU_GROUP) { $env:POPULAR_MENU_GROUP } else { 'popular-menu-load-test' }
    PollInterval = if ($env:OUTBOX_POLL_INTERVAL) { $env:OUTBOX_POLL_INTERVAL } else { '200ms' }
