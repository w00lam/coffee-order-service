...
foreach ($port in $LoadTestConfig.AppPorts | Where-Object { $_ -ne $failedPort }) { Wait-HttpReady -Port $port }
...
& (Join-Path $PSScriptRoot 'verify-invariants.ps1') -RunPrefix $RunPrefix -ExpectedChargeBalance 0 -TimeoutSeconds 120
